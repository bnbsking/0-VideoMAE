...
def main(args):
    print(args); import os,json,glob; os.makedirs(args.save_path,exist_ok=True); json.dump( vars(args), open(f"{args.save_path}/args.json","w") )#; raise
    ...
    for path in glob.glob(f"{args.img_path}/*.mp4")[:3]:
        ...
        if True:
            #print(len(vr)); raise # 300 # modeling_finetune.py line138
            tmp = np.arange(0,300,4)
            tmp = tmp[:-1] if len(tmp)%2 else tmp
            args.num_frames = len(tmp)
            args.window_size = (len(tmp)//2,14,14)
            import cv2
            def imgs2video(imgL,name):
                fourcc = cv2.VideoWriter_fourcc(*'MP4V')
                out = cv2.VideoWriter(f"{args.save_path}/{path.split('/')[-1].replace('.mp4','_'+name+'.mp4')}", fourcc, 7, (224,224))
                for img in imgL:
        ...
        bool_masked_pos = torch.from_numpy(bool_masked_pos) # (1568,) # 1408 0, 160 1
